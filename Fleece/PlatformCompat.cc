...
#define MIGRATE_1(from, to) fleece::from(const char* const arg1) { \
CA2WEX<256> wide(arg1, CP_UTF8); \
return to(wide); \
}

#define MIGRATE_2(from, to, type1) fleece::from(const char* const arg1, type1 arg2) { \
CA2WEX<256> wide(arg1, CP_UTF8); \
return to(wide, arg2); \
}

#define MIGRATE_2S(from, to) fleece::from(const char* const arg1, const char* const arg2) { \
...
int fleece::mkdir_u8(const char *path, int mode) {
    MIGRATE_ARG(path, ::_wmkdir(wpath));
}

int fleece::stat_u8(const char* const filename, struct stat * const s)
{
