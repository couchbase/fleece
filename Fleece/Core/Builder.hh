...
namespace fleece::impl::builder {
    ...
    /** Variant of Build that writes the value to an Encoder.
        Format string syntax is the same, except that the braces around the outer dictionary may be removed,
        which causes its keys and values to be added to the Encoder's current dictionary. */
    void Encode(Encoder&, const char *format, ...) __printflike(2, 3);
    void Encode(JSONEncoder&, const char *format, ...) __printflike(2, 3);
    ...
    /** Variant of Build to an Encoder that allows `%@` for [Core]Foundation values; the corresponding arg
        ...
        \note The format string is a `CFStringRef` not a `char*`, because `CF_FORMAT_FUNCTION`
        requires that. */
    void EncodeCF(Encoder&, CFStringRef format, ...) CF_FORMAT_FUNCTION(2, 3);
#endif
}
