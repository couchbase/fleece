...
__printflike(1, 2) static RetainedConst<Value> buildWithEncoder(const char* format, ...)  {
    Encoder enc;
    va_list args;
    va_start(args, format);
    builder::VEncode(enc, format, args);
    va_end(args);
    return enc.finishDoc()->root();
}


TEST_CASE("Builder Encode Literals", "[Builder]") {
    auto v = buildWithEncoder("[null, false, true, 0, 1, -12, +123, 123.5, -123.5, +123.5, 123e-4]");
    REQUIRE(v);
    CHECK(v->toJSONString() == "[null,false,true,0,1,-12,123,123.5,-123.5,123.5,0.0123]");
}


TEST_CASE("Builder Encode String Literals", "[Builder]") {
    auto v = buildWithEncoder(R"({a : 'foo\'', $b : "bar\"rab", _c_ : "", _ : "\r\\"})");
    REQUIRE(v);
    std::string expected = R"({"$b":"bar\"rab","_":"\r\\","_c_":"","a":"foo'"})";
    CHECK(v->toJSONString() == expected);
}
...
TEST_CASE("Builder Encode Basic Dict", "[Builder]") {
    auto v = buildWithEncoder("{name:%s, size:%d, weight:%f}",
                            "Zegpold", 12, 3.14);
    auto dict = v->asDict();
    REQUIRE(dict);
    CHECK(dict->get("name")->asString() == "Zegpold");
...
TEST_CASE("Builder Encode Basic Array", "[Builder]") {
    auto v = buildWithEncoder("[%s, %d, %f]",
                            "Zegpold", 12, 3.14);
    auto array = v->asArray();
    REQUIRE(array);
    CHECK(array->get(0)->asString() == "Zegpold");
...
TEST_CASE("Builder Encode Nesting", "[Builder]") {
    auto v = buildWithEncoder("{name:%s, coords:[%d, %d], info:{nickname:%s}}",
                            "Zegpold", 4, 5, "Zeggy");
    CHECK(v->toJSONString() == R"({"coords":[4,5],"info":{"nickname":"Zeggy"},"name":"Zegpold"})");
}


TEST_CASE("Builder Encode Bool Params", "[Builder]") {
    bool t = true, f = false;
    auto v = buildWithEncoder("[%c,%c]", char(t), char(f));
    CHECK(v->toJSONString() == R"([true,false])");
}


TEST_CASE("Builder Encode Integer Params", "[Builder]") {
    int i0 = INT_MIN, i1 = INT_MAX;
    unsigned u = UINT_MAX;
    long l0 = LONG_MIN, l1 = LONG_MAX;
    unsigned long ul = ULONG_MAX;
    ...
    ptrdiff_t p0 = PTRDIFF_MIN, p1 = PTRDIFF_MAX;
    size_t z1 = SIZE_MAX;
    auto v = buildWithEncoder("[[%d, %d, %u], [%ld,%ld,%lu], [%lld,%lld,%llu], [%zd,%zd,%zu]]",
                               i0, i1, u,     l0, l1, ul,    ll0, ll1, ull,    p0, p1, z1);
    std::string expected32 = "[-2147483648,2147483647,4294967295]";
    std::string expected64 = "[-9223372036854775808,9223372036854775807,18446744073709551615]";
    std::string expected =
...
TEST_CASE("Builder Encode Value Params", "[Builder]") {
    auto v1 = buildWithEncoder("[%s, %d, %f]",
                             "Zegpold", 12, 3.14);
    auto v2 = buildWithEncoder("{v1: %p, v2: %p}", v1.get(), v1.get());
    CHECK(v2->toJSONString() == R"({"v1":["Zegpold",12,3.14],"v2":["Zegpold",12,3.14]})");
}


TEST_CASE("Builder Encode Empty Strings", "[Builder]") {
    const char *str = "";
    slice sl(str);
    auto v = buildWithEncoder("{a:%s, b:%.*s, d:[%s, %.*s]}",
                            str, FMTSLICE(sl), str, FMTSLICE(sl));
    CHECK(v->toJSONString() == R"({"a":"","b":"","d":["",""]})");
}


TEST_CASE("Builder Encode Null Args", "[Builder]") {
    const char *str = nullptr;
    slice sl = nullslice;
    const Value *val = nullptr;
    auto v = buildWithEncoder("{a:%s, b:%.*s, c:%p, d:[%s, %.*s, %p]}",
                            str, FMTSLICE(sl), val, str, FMTSLICE(sl), val);
    CHECK(v->toJSONString() == R"({"d":[]})");
}


TEST_CASE("Builder Encode Default Suppression", "[Builder]") {
    const char *str = "";
    slice sl(str);
    auto v = buildWithEncoder("[%-c, %-d, %-f, %-s, %-.*s]",
                            char(false), 0, 0.0, str, FMTSLICE(sl));
    CHECK(v->toJSONString() == R"([])");
}


TEST_CASE("Builder Encode Into Existing Dict", "[Builder]") {
    Encoder enc;
    enc.beginDictionary();
    enc.writeKey("A");
    enc.writeInt(1);

    builder::Encode(enc, "name:%s, size:%d, weight:%f", // Missing braces means encoding into open dict
                    "Zegpold", 12, 3.14);
    ...
    auto dict = doc->asDict();
    REQUIRE(dict);
    CHECK(dict->get("A")->asInt() == 1);
    CHECK(dict->get("name")->asString() == "Zegpold");
    CHECK(dict->get("size")->asInt() == 12);
    CHECK(dict->get("weight")->asDouble() == 3.14);
    CHECK(dict->get("Z")->asInt() == 26);
    CHECK(dict->toJSONString() == R"({"A":1,"Z":26,"name":"Zegpold","size":12,"weight":3.14})");
}
